...
public final class EntityPowerWave extends EntityThrowable
{
   ...
   public void onUpdate() {
      ...
      if(this.counter % 2 == 0 && this.counter > 1) {
         ...
         for(int l = j; l > 1; --l) {
            Block list = this.worldObj.getBlock(i, l, k);
            if(list != Blocks.air) {
               j = l;
               break;
            }
         ...
         List var8 = this.worldObj.getEntitiesWithinAABB(EntityLiving.class, AxisAlignedBB.getBoundingBox(this.posX, this.posY, this.posZ, this.posX + 1.0D, this.posY + 1.0D, this.posZ + 1.0D).expand(5.0D, 5.0D, 5.0D));
         if(!var8.isEmpty()) {
            boolean flag = true;
            EntityLivingBase entityliving = (EntityLivingBase)var8.get(this.worldObj.rand.nextInt(var8.size()));
